...
def main():
    ...
    model_inception = torchvision.models.inception_v3(weights=Inception_V3_Weights.DEFAULT)
    model_inception.eval()
    model_inception.to(device)
    model_inception = create_feature_extractor(model_inception, return_nodes={'flatten': 'flatten'})

    # AlexNet
    model_alexnet = torchvision.models.alexnet(weights=AlexNet_Weights.DEFAULT)
    model_alexnet.eval()
    model_alexnet.to(device)
    model_alexnet = create_feature_extractor(
            model_alexnet,
            return_nodes={
